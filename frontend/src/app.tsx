...
import { UserAuditPage } from './features/activity/components/user-audit-page';
...
function AuthenticatedApp({ onLogout }: { onLogout: () => void }) {
  ...
  const reload = () => { cameras.reload(); recordings.reload(); setActivityRefreshToken(value => value + 1); };
  ...
  const openCamera = (camera: Camera) => {
    ...
    void api.logCameraOpened(camera.id).catch(() => undefined);
  };

  const toggleAlarm = (camera: Camera, enabled: boolean) => {
    void updateAlarmPreference(camera.id, camera.name, enabled).catch(() => undefined);
  };
  ...
  return <div className="app-shell">
    ...
    <main className="app-main">
      ...
      {currentView === 'activity' && <UserAuditPage refreshToken={activityRefreshToken}/>}
