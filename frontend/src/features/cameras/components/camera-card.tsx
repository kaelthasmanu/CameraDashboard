...
import { Bell, BellOff, Camera as CameraIcon, MoreVertical, Play, RotateCw, WifiOff } from 'lucide-react';
...
export function CameraCard({ camera, onSelect, streamActive = true, alarmEnabled = true, onAlarmToggle }: { camera: Camera; onSelect: (camera: Camera) => void; streamActive?: boolean; alarmEnabled?: boolean; onAlarmToggle?: (enabled: boolean) => void }) {
  return <article className="camera-card" onClick={() => onSelect(camera)} role="button" tabIndex={0} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') onSelect(camera); }}>
    <div className="feed">
      ...
      {camera.status === 'online' && camera.enabled ? <CameraStream camera={camera} compact active={streamActive} alarmEnabled={alarmEnabled}/> : <div className="offline"><WifiOff size={30}/><span>Cámara desconectada</span><small>{camera.last_seen ? `Última conexión: ${formatDate(camera.last_seen)}` : 'Sin conexión registrada'}</small></div>}
...
export function CameraStream({ camera, compact = false, active = true, alarmEnabled = true }: { camera: Camera; compact?: boolean; active?: boolean; alarmEnabled?: boolean }) {
  ...
  const motionDetected = useMotionAlarm(videoRef, alarmEnabled && enabled);
  ...
  return <div className={`scene ${compact ? 'compact' : ''} ${motionDetected ? 'motion-detected' : ''}`}>
    ...
    {motionDetected && <div className="motion-alert" role="status"><Bell size={14}/> Movimiento detectado</div>}
